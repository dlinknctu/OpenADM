...
try:
  ...
  weblog = core.getLogger("upload_webcore.server")
...
dpid_cannot_use = []  #record the dpid has been used
dpid_used = []


class StatsEvent_Listener (EventMixin):
  ...
  def _handle_StatsEvent (self, event):
    self.ports = event.ports_data
    self.flows = event.flows_data
    self._json_format_translation()
    return
        ...
        for p_index in self.ports:
          if p_index['dpid'] == f_index['dpid'] :
            if f_index['dpid'] not in dpid_cannot_use :
              ...
              file_nodes_dict['dpid'] = p_index['dpid']

              n_temp = file_nodes_dict.copy()            
              file_nodes.append(n_temp)
                
              file_ports[:] = []
              file_flows[:] = []
              file_nodes_dict.clear()

      
      """
      dpid_used is the same to dpid_cannot_use ?
      
      """
      s1 = set(dpid_used)
      s2 = set(dpid_cannot_use)
