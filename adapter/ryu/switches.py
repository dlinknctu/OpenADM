...
from ryu.lib.packet import arp, ipv4, ipv6, vlan
...
class Port(object):
    ...
    def __init__(self, dpid, ofproto, ofpport):
        ...
        self.currentFeatures = ofpport.curr
...
class HostState(dict):
    ...
    def update_vlan(self, host, vlan=None):
        mac = host.mac
        host = None
        if mac in self:
            host = self[mac]
...
class Switches(app_manager.RyuApp):
    ...
    def host_discovery_packet_in_handler(self, ev):
        ...
        # vlan packet, update vlan tag
        vlan_pkt = pkt.get_protocol(vlan.vlan)
        if vlan_pkt:
            ans = self.hosts.update_vlan(host, vlan=vlan_pkt.vid)
            if ans == 'new':
                ev = event.EventHostAdd(host)
                self.send_event_to_observers(ev)
